...
with st.sidebar:
    ...
    def export_emotion_report():
        ...
        lines = [
            ...
            f"",
            f"## 基本信息",
            f"- 记录时间：{dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
            f"- 对话编号：{sess.session_id}",
            f"- 对话轮数：{ctx.total_turns}",
            f"- 你说的话：{len(user_msgs)} 条",
            f"- 求助信号次数：{ctx.help_seeking_count} 次",
            f"- 主要情绪：{ctx.dominant_sentiment}",
            f"- 连续不开心轮次：{ctx.negative_streak}",
            f"- 情绪波动：{'是' if ctx.escalation_flag else '否'}",
            f"",
            f"## 心情变化趋势",
        ]
        
        if ctx.sentiment_trend:
            lines.append(f"- 心情变化序列：{ctx.sentiment_trend}")
            lines.append(f"- 平均心情指数：{np.mean(ctx.sentiment_trend):.1f}")
            lines.append(f"- 最高心情指数：{max(ctx.sentiment_trend)}")
            lines.append(f"- 最低心情指数：{min(ctx.sentiment_trend)}")
            risk_turns = [i+1 for i, r in enumerate(ctx.risk_history) if r]
            if risk_turns:
                lines.append(f"- ⚠️ 需要关注的轮次：{risk_turns}")
        
        lines.append(f"")
        lines.append(f"## 每轮对话记录")
        
        for i, msg in enumerate(user_msgs, 1):
            r = msg.analysis
            lines.append(f"")
            lines.append(f"### 第 {i} 轮 ({msg.timestamp.strftime('%H:%M:%S')})")
            lines.append(f"**你说的话**：{msg.content}")
            lines.append(f"- 心情：{r['情绪极性']['label']} (命中: {', '.join(r['情绪极性']['matched'][:5]) or '无'})")
            ...
            lines.append(f"- 心情指数：{r['情感强度']['强度']} (净得分: {r['情感强度']['净得分']})")
            
            ctx_info = r.get("上下文", {})
            hints = ctx_info.get("上下文提示", [])
            if hints:
                lines.append(f"- 温馨提示：{'; '.join(hints)}")
        
        lines.append(f"")
        lines.append(f"## 温馨提示")
        
        if ctx.escalation_flag:
            lines.append(f"⚠️ **你的情绪变化比较大**，建议和身边信任的人聊聊，或者寻求专业帮助。")
        if ctx.negative_streak >= 3:
            lines.append(f"⚠️ **你已经连续 {ctx.negative_streak + 1} 次表达不开心了**，希望你能得到更多的关心和支持。")
        if ctx.help_seeking_count > 0:
            lines.append(f"✅ 你表达了 {ctx.help_seeking_count} 次求助信号，这很勇敢，希望你能得到及时的帮助。")
        
        lines.append(f"")
        lines.append(f"> 这份记录仅供参考，不能替代专业的心理咨询。如果你正在经历困难，请拨打全国心理援助热线 12356 或 120。")
    ...
    if user_msgs:
        ...
        with st.expander("查看分析细节"):
            ...
            net = r["情感强度"]["净得分"]
            st.markdown(f"**心情指数**：净得分 {net}（积极 {sum(pos_hits.values())} / 消极 {sum(neg_hits.values())}）")
    else:
        st.info("还没有对话，说点什么吧～")
...
if len(ctx.sentiment_trend) >= 2:
    ...
    trend_ax.set_title("你的心情变化", fontsize=12)
    ...
    for msg in sess.messages:
        if msg.role == "user":
            ...
            st.markdown(
                ...
                <div class="chat-ai">
                  <div>{msg.content}</div>
                  <div class="chat-meta">小树洞 · {msg.timestamp.strftime('%H:%M:%S')}</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
...
st.markdown("**试试聊聊**（按顺序点击，模拟一段真实对话）：")
demo_cols = st.columns(3)
scenarios = [
    ("① 最近好累", "最近总觉得活着好累，什么都不想干，特别绝望……"),
    ("② 想放弃了", "嗯…工作也辞了，天天在家躺着，感觉人生没有希望了。"),
    ("③ 求助信号", "有时候甚至想过结束一切，我该怎么办，能救救我吗？"),
    ("④ 看医生了", "医生告诉我需要长期服药，每次复诊都很焦虑等待结果，真的好难熬"),
    ("⑤ 家里压力", "工作压力加上家人的期待让我喘不过气，觉得自己在两头都难顾"),
    ("⑥ 治疗进展", "患者在治疗第6周出现改善，情绪更稳定但仍有间歇性焦虑发作"),
]
...
st.markdown(
    ...
    <div style="color:#9e9e9e;font-size:12px;">
      这是一个学术研究项目的情感陪伴演示。对话内容由 AI 生成，仅供参考，
      <strong>不能替代专业的心理咨询</strong>。如果你正在经历困难，请拨打
      <strong>全国心理援助热线 12356</strong> 或 <strong>120</strong>，会有人帮助你。
    </div>
    """,
    unsafe_allow_html=True,
)
